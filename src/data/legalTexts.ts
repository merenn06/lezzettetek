export const TESLIMAT_VE_IADE_SARTLARI = `**TESLİMAT VE İADE ŞARTLARI**

**1. Teslimat**

* Siparişleriniz, ödeme onayı alındıktan sonra hazırlanır ve kargoya teslim edilir.
* Kargo teslim süresi, kargo firmasının yoğunluğuna ve teslimat adresine göre değişmekle birlikte genellikle **1–5 iş günü** içerisindedir.
* Resmî tatiller ve hafta sonları kargo teslimatı yapılmayabilir.
* Teslimat sırasında alıcının adreste bulunmaması halinde kargo firması paketi şubede bekletebilir; teslim alınmayan gönderiler iade olabilir.

**2. Kargo Ücretleri**

* Kargo ücreti ve varsa ücretsiz kargo koşulları, ödeme adımında sepet ekranında gösterilir.

**3. Teslimat Kontrolü**

* Paketi teslim alırken dış ambalajda ezilme/yırtılma/ıslaklık gibi hasar varsa, kargo görevlisi yanında tutanak tutturarak paketi teslim almayınız.
* Tutanaksız teslim alınan hasarlı ürünlerden doğabilecek sorunlarda süreç uzayabilir.

**4. İade ve Cayma Hakkı**

* Mesafeli satışlarda tüketicinin cayma hakkı bulunmaktadır.
* **Gıda ürünleri** niteliği gereği ve hijyen/sağlık gerekçeleriyle, ambalajı açılmış/bozulmuş/yeniden satılabilirliği kaybolmuş ürünlerde cayma hakkı istisna kapsamına girebilir.
* İade talebiniz için bizimle iletişime geçmeniz gerekir.

**5. İade Süreci**

* İade/iptal taleplerinizi e-posta yoluyla iletebilirsiniz.
* İade onayı verilen ürünler, tarafımızdan bildirilen kargo yöntemiyle gönderilmelidir.
* İade kargosu satıcıya ulaştıktan sonra inceleme yapılır; uygun bulunması halinde iade işlemi başlatılır.

**6. Ücret İadesi**

* İade onaylandığında ücret iadesi, ödeme yöntemine göre bankanıza bağlı olarak **1–10 iş günü** içinde hesabınıza yansıyabilir.

**İletişim**
E-posta: **[info@teklezzet.com](mailto:info@teklezzet.com)**`;

export const GIZLILIK_SOZLESMESI = `**GİZLİLİK SÖZLEŞMESİ**

Bu metin, web sitemiz üzerinden paylaşılan kişisel verilerin işlenmesi ve korunmasına ilişkin bilgilendirme amacıyla hazırlanmıştır.

**1. Toplanan Veriler**
Sipariş ve hizmetlerin sunulabilmesi için aşağıdaki bilgiler işlenebilir:

* Ad, soyad
* Telefon numarası
* E-posta adresi
* Teslimat/fatura adresi
* Sipariş ve ödeme bilgileri (kart bilgileri sitemizde saklanmaz)

**2. Verilerin Kullanım Amaçları**

* Sipariş süreçlerinin yürütülmesi
* Kargo/teslimat işlemlerinin yapılması
* Müşteri destek süreçlerinin yürütülmesi
* Yasal yükümlülüklerin yerine getirilmesi

**3. Ödeme Güvenliği**
Ödemeler, **iyzico** gibi lisanslı ödeme hizmeti sağlayıcıları üzerinden gerçekleştirilir. Kart bilgileriniz site altyapımızda tutulmaz; ödeme sağlayıcının güvenli altyapısı üzerinden işlenir.

**4. Üçüncü Taraflarla Paylaşım**
Kişisel veriler;

* kargo firmaları (teslimat için),
* ödeme hizmeti sağlayıcıları (ödeme işlemi için),
* yasal zorunluluk hallerinde yetkili kurumlarla
  paylaşılabilir.

**5. Çerezler (Cookies)**
Site deneyimini geliştirmek amacıyla çerezler kullanılabilir. Tarayıcı ayarlarından çerezleri yönetebilir veya silebilirsiniz.

**6. Saklama Süresi**
Veriler, ilgili mevzuatta öngörülen süreler boyunca veya işleme amacının gerektirdiği süre kadar saklanır.

**7. Haklarınız**
KVKK kapsamında; verilerinizin işlenip işlenmediğini öğrenme, düzeltme, silme, itiraz etme gibi haklara sahipsiniz. Talepleriniz için bize e-posta ile ulaşabilirsiniz.

**İletişim**
E-posta: **[info@teklezzet.com](mailto:info@teklezzet.com)**`;

export const ON_BILGILENDIRME_VE_MESAFELI_SOZLESME = `**MESAFELİ SATIŞ SÖZLEŞMESİ**

İşbu sözleşme, alıcı ile satıcı arasında aşağıdaki koşullarda elektronik ortamda kurulmuştur.

**1. Taraflar**
**Satıcı:** FS TARIM ÜRÜN. (Tek Lezzet / Lezzette Tek)
**E-posta:** [info@teklezzet.com](mailto:info@teklezzet.com)

**Alıcı:** Site üzerinden sipariş veren kişi (müşteri)

**2. Sözleşmenin Konusu**
İşbu sözleşmenin konusu, alıcının satıcıya ait web sitesi üzerinden sipariş verdiği ürün/ürünlerin satışı ve teslimine ilişkin tarafların hak ve yükümlülüklerinin belirlenmesidir.

**3. Ürün Bilgileri ve Fiyat**
Ürün adı, adedi, satış bedeli, kargo ücreti ve toplam tutar; sipariş anında alıcı tarafından onaylanan sepet/ödeme ekranında yer aldığı şekildedir.

**4. Ödeme**
Ödemeler, güvenli ödeme altyapısı üzerinden alınır. Ödeme onayı ile sipariş işleme alınır.

**5. Teslimat**

* Ürün/ürünler, alıcının sipariş sırasında belirttiği teslimat adresine kargo ile gönderilir.
* Teslimat süresi genellikle **1–5 iş günü** olmakla birlikte, bölge ve kargo yoğunluğuna göre değişebilir.

**6. Cayma Hakkı**

* Alıcı, mesafeli sözleşmelerde cayma hakkına sahiptir.
* Ancak mevzuat gereği; **hızla bozulabilen, son kullanma tarihi geçebilen, hijyen nedeniyle iadesi uygun olmayan gıda ürünlerinde** cayma hakkı istisna kapsamında olabilir.
* Cayma/iptal talepleri için alıcı, satıcı ile iletişime geçmelidir.

**7. İade Şartları**
İade onayı verilen ürünlerin, satıcının bildireceği yöntemle iade edilmesi gerekir. Ürün satıcıya ulaştıktan sonra kontrol edilir ve uygun bulunması halinde iade süreci başlatılır.

**8. Uyuşmazlıkların Çözümü**
Tüketici işlemlerinden doğabilecek uyuşmazlıklarda, alıcının yerleşim yerindeki veya işlemin yapıldığı yerdeki **Tüketici Hakem Heyetleri** ve **Tüketici Mahkemeleri** yetkilidir.

**9. Yürürlük**
Alıcı, site üzerinden siparişi onaylayarak işbu sözleşme hükümlerini kabul etmiş sayılır.

**İletişim**
E-posta: **[info@teklezzet.com](mailto:info@teklezzet.com)**`;

export const KVKK_AYDINLATMA_METNI = `KİŞİSEL VERİLERİN KORUNMASI AYDINLATMA METNİ (KVKK)

Bu Aydınlatma Metni, 6698 sayılı Kişisel Verilerin Korunması Kanunu ("KVKK") uyarınca, veri sorumlusu sıfatıyla FS TARIM ÜRÜN. – Ferhat Çelik tarafından hazırlanmıştır.

**1. Veri Sorumlusu**

Unvan: FS TARIM ÜRÜN. – Ferhat Çelik
Marka: Tek Lezzet / Lezzette Tek
E-posta: info@teklezzet.com

**2. İşlenen Kişisel Veriler**

Web sitemiz üzerinden sunulan hizmetler kapsamında aşağıdaki kişisel veriler işlenebilmektedir:

Kimlik bilgileri (ad, soyad)

İletişim bilgileri (telefon numarası, e-posta adresi)

Adres bilgileri (teslimat ve fatura adresi)

Sipariş ve işlem bilgileri

Ödeme bilgilerine ilişkin işlem verileri

(Kredi/banka kartı bilgileri tarafımızca saklanmaz)

**3. Kişisel Verilerin İşlenme Amaçları**

Kişisel verileriniz aşağıdaki amaçlarla işlenmektedir:

Sipariş ve satış süreçlerinin yürütülmesi

Ürünlerin kargo yoluyla teslim edilmesi

Ödeme işlemlerinin gerçekleştirilmesi

Müşteri destek ve iletişim süreçlerinin yürütülmesi

Yasal yükümlülüklerin yerine getirilmesi

**4. Kişisel Verilerin Aktarılması**

Kişisel verileriniz, yukarıda belirtilen amaçlarla sınırlı olmak üzere;

Kargo firmalarına (teslimat işlemleri için),

Ödeme hizmeti sağlayıcılarına (iyzico vb.) (ödeme işlemleri için),

Yetkili kamu kurum ve kuruluşlarına (yasal zorunluluklar kapsamında)

aktarılabilmektedir.

**5. Kişisel Verilerin Toplanma Yöntemi ve Hukuki Sebebi**

Kişisel verileriniz;

Web sitemiz üzerindeki formlar,

Sipariş ve ödeme ekranları,

E-posta ve elektronik iletişim kanalları

aracılığıyla otomatik veya kısmen otomatik yöntemlerle toplanmaktadır.

Hukuki sebepler:

Bir sözleşmenin kurulması veya ifası

Veri sorumlusunun hukuki yükümlülüğünü yerine getirmesi

Meşru menfaatler

Açık rıza (gerektiği durumlarda)

**6. KVKK Kapsamındaki Haklarınız**

KVKK'nın 11. maddesi uyarınca veri sahipleri olarak:

Kişisel verilerinizin işlenip işlenmediğini öğrenme

İşlenmişse buna ilişkin bilgi talep etme

Amacına uygun kullanılıp kullanılmadığını öğrenme

Eksik veya yanlış işlenmişse düzeltilmesini isteme

Kanuna uygun olarak silinmesini veya yok edilmesini isteme

İşlemlerin aktarıldığı üçüncü kişilere bildirilmesini isteme

Otomatik sistemler ile analiz edilmesi sonucu aleyhinize bir sonucun ortaya çıkmasına itiraz etme

haklarına sahipsiniz.

**7. Başvuru Yolu**

KVKK kapsamındaki taleplerinizi, yazılı olarak veya e-posta yoluyla bize iletebilirsiniz:

📧 info@teklezzet.com

Başvurularınız, mevzuata uygun olarak en geç 30 gün içinde sonuçlandırılacaktır.`;

